...
<?php
    ...
    {
        $id = $_GET['edit_id'];
        $stmt_edit = $DB_con->prepare('SELECT title,img,description,button,link FROM slider WHERE id =:id');
    ...
    if(isset($_POST['btn_save_updates']))
    {
     ...
     $link = $_POST['link'];
        ...
        if(!isset($errMSG))
        {
$stmt = $DB_con->prepare('UPDATE slider SET img=:img, title=:title, description=:description, button=:button, link=:link
    WHERE id=:id');
