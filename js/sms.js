...
async function verifyMobileNo(event) {
    try {
        ...
        mobileTextBox.prop('disabled', true);
        ...
        if (await sendOTP(userRegistrationMobile)) {
            ...
            $(formName).find('.resend-otp-button').prop('disabled', true);
            ...
            mobileTextBox.prop('disabled', false);
    ...
    try {
        otpTimeout = window.setInterval(() => {
            if (timeInSeconds <= 0) {
                ...
                $(context).find('.user-verify-mobile').prop('disabled', false);

                // disable otp textbox
                $(context).find('.user-verify-otp').prop('disabled', true);

                // enable resend otp button
                $(context).find('.resend-otp-button').prop('disabled', true);
